var host = "https://testapi.io/api/Nam/210";
    ...
    $.ajax({
        ...
        success: function (d) {
            ...
            for (let i = 0; i < d.length; i++) {
                let tr = '<tr>';
                let td = '<td>';
                let html = tr + td +  d[i].name + td + '<img class="img1" src="' + d[i].picture+ '">'+ td + d[i].massv + td + d[i].lop + td + d[i].toan + td + d[i].ly  + td + d[i].hoa ;
